...
MapsTables - A standalone add-on to create and maintain Map tables for the QuestMap2 project
...
----------------------------------------------------------------------------------------------------------------------------------]]
local MAJOR, MINOR = "MapsTables", 100
...
if addon then
	assert( version < MINOR, "MapsTables: This add-on is already loaded. Do NOT load MapsTables multiple times!" )
end

addon, version = AddonStub:New( MAJOR, MINOR )
assert( addon, "MapsTables: AddonStub failed to create a control entry!" )
...
assert( GPS, "MapsTables: LibStub refused to create a link to LibGPS2!" )
...
local function LoadOneMap( mdx: number )			-- The Map Index of this Map
	...
	assert( GetCurrentMapIndex() == mdx, "MapsTables:LoadOneMap, mapIndexes are not equal!" )
...
--[[--------------------------------------------------------------------------------------------------------------------------------
The "OnAddonLoaded" function reads the saved variables table (sv) from the saved variables file, "...\SavedVariables\MapsTables.lua"
...
links everything in its local tables to their equivalent MapsTables table entries.
----------------------------------------------------------------------------------------------------------------------------------]]
local function OnAddonLoaded( event, name )
	...
	assert( GPS:IsReady(), "MapsTables: LibGPS2 cannot function until a player is active!" )
	...
	sv.numMaps = numMaps
	-- Create a new or update an existing Saved Variables data file (e.g. "...\SavedVariables\MapsTables.lua")
	_G[savedVarsFile] = sv
end

--[[--------------------------------------------------------------------------------------------------------------------------------
MapsTables public API function definitions. MapsTables does not duplicate any of the game API functions listed below:
